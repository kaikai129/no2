...
const ENABLE_UPLOAD = parseBooleanEnv('ENABLE_UPLOAD'); // true时才允许向UPLOAD_URL上传订阅或节点
const ENABLE_META_LOOKUP = parseBooleanEnv('ENABLE_META_LOOKUP'); // true时才请求第三方IP/ISP接口生成节点名
const FILE_PATH = process.env.FILE_PATH || '.tmp';   // 运行目录,sub节点文件保存目录
const SUB_PATH = (process.env.SUB_PATH || 'hub').replace(/^\/+/, '');       // 订阅路径
const SUB_TOKEN = process.env.SUB_TOKEN || crypto.randomBytes(24).toString('hex'); // 订阅访问密钥,未设置则每次启动随机生成
...
function buildSubscriptionPath() {
  return `/${SUB_PATH}?token=${encodeURIComponent(SUB_TOKEN)}`;
}

function buildSubscriptionUrl(baseUrl) {
  return `${baseUrl.replace(/\/+$/, '')}${buildSubscriptionPath()}`;
}

function isSubscriptionAuthorized(req) {
  const authorization = req.get('authorization') || '';
  const bearerMatch = authorization.match(/^Bearer\s+(.+)$/i);
  const queryToken = Array.isArray(req.query.token) ? req.query.token[0] : req.query.token;
  const providedToken = String(queryToken || (bearerMatch ? bearerMatch[1] : ''));
  const expected = Buffer.from(SUB_TOKEN);
...
async function generateLinks(argoDomain) {
  ...
  const nodeName = NAME && ISP ? `${NAME}-${ISP}` : NAME || ISP || 'Node';
...
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${argoDomain}&fp=firefox&type=ws&host=${argoDomain}&path=%2Ftrojan-argo%3Fed%3D2560#${nodeName}
    `;
      ...
      if (!process.env.SUB_TOKEN) {
        console.log('SUB_TOKEN is not set; generated a random token for this run');
      }
      console.log(`Subscription URL: https://${argoDomain}${buildSubscriptionPath()}`);
      uploadNodes();
...
async function uploadNodes() {
  if (!ENABLE_UPLOAD) {
    console.log('Skipping upload nodes: ENABLE_UPLOAD is not true');
    return;
  }

  if (UPLOAD_URL && PROJECT_URL) {
    const subscriptionUrl = buildSubscriptionUrl(PROJECT_URL);
    const jsonData = {
